...
def getWordScore(word, n):
    ...
    # get frequency of each letter used in word
    freq = getFrequencyDict(word)

    # get score for each letter used and multiply by frequency
    for key, num in freq.items():
        letterScore = SCRABBLE_LETTER_VALUES[key] * len(word) * num

        # add score to total
        score += letterScore

    # bonus if full hand used
    if len(word) == n:
        score += 50
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    while calculateHandlen(hand) > 0:
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                score += wScore
                print('"' + word + '" earned', wScore, 'points. Total:',
                      score, 'points')
                print()
    ...
    if word == '.':
        print('Goodby! Total score:', score, 'points.')
    else:
...
def playGame(wordList):
    ...
    while True:
        ...
        # for 'n', deal new hand and play one round
        if choice == 'n':
            hand = dealHand(HAND_SIZE)
            playHand(hand, wordList, HAND_SIZE)

        # for 'r', play round with last hand, if one exists
        elif choice == 'r':
            try:
                playHand(hand, wordList, HAND_SIZE)
            except NameError:
